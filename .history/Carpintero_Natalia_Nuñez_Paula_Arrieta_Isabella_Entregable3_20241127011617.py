...
class PerfectMatch: # Clase principal
    ...
    def leer_empleados(self,archivo_empleados): # Función para leer empleados.txt
        ...
        with open(archivo_empleados, 'r') as file: #abrir archivo
            for line in file:
                ...
                empleados.append((nombre, ocupacion, int(precio_hora)))
        return empleados
    ...
    def leer_clientes(self, archivo_clientes): # Función para leer clientes.txt
        ...
        with open(archivo_clientes, 'r') as file:
            for line in file:
                ...
                clientes.append((nombre, ocupacion_requerida, int(presupuesto)))
        return clientes
    ...
    def crear_grafo(self): # Grafo (matriz de adyacencia)
        ...
        for i, empleado in enumerate(self.empleados): # Recorrer empleados
            for j, cliente in enumerate(self.clientes): # Recorrer clientes
                '''Si la ocupación del empleado es igual a la ocupación requerida del cliente 
                y el precio del empleado es menor o igual al presupuesto del cliente'''
    ...
    def output(self):
        matchR, total = self.maxBPM()
        emparejamientos = []
        for j in range(len(self.clientes)): # Recorrer clientes
            if matchR[j] != -1: # Si el cliente tiene un empleado asignado
                # Se añade a la lista de emparejamientos el nombre del cliente y el nombre del empleado asignado
                emparejamientos.append(f"{self.clientes[j][0]} - {self.empleados[matchR[j]][0]}")
        print("Emparejamientos:")
        for emp in emparejamientos:
            print(emp) # Se imprime cada emparejamiento
        print(f"Total de emparejamientos realizados: {total}") # Se imprime el total de emparejamientos realizados
    
# INICIO
print("Bienvenidx a PerfectMatch")
perfectM = PerfectMatch("empleados4.txt", "clientes4.txt") 
perfectM.output()
